function readUserIdSequence(){
	...
	rs = conn.executeQuery('SELECT \"sap.hana.democontent.epm.data::userSeqId\".NEXTVAL as OverallId from Dummy');
	overAllId = '';
	if (rs.length !== 0) {
...
function clearJobLogs(){
    ...
    var conn = $.hdb.getConnection();
    try{
        conn.executeUpdate('DELETE FROM "sap.hana.democontent.epm.data::User.Details"');
        conn.commit();
        conn.close();
    }
    catch(e){
