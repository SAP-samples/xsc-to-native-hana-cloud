...
query = 'SELECT \"sap.hana.democontent.epm.data::salesOrderId\".NEXTVAL as OverallId from Dummy';
...
query = 'SELECT COMPANYNAME from "sap.hana.democontent.epm.data::MD.BusinessPartner" where PARTNERID = ?';
...
if (so_items) {
    ...
    for (i; i < so_items.length; i++) {
        ...
        query = 'SELECT PRICE from "sap.hana.democontent.epm.data::MD.Products" where PRODUCTID = ?';
        ...
        taxCalculator = conn.loadProcedure('sap.hana.democontent.epm.models::DT_TAX_CALCULATION');
        ...
        query = 'insert into "sap.hana.democontent.epm.data::SO.Item"' + " values(?,?,?,'','EUR',?,?,?,'I','',?,'EA','')";
        finalItemId = '';
        if (item_id >= 100) {
...
query = 'insert into \"sap.hana.democontent.epm.data::SO.Header\"' 
