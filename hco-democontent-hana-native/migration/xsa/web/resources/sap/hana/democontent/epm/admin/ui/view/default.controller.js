sap.ui.controller("sap.hana.democontent.epm.admin.ui.view.default", {
    /**
    ...
    onInit: function () {
        ...
        var oDataset = new sap.viz.ui5.data.FlattenedDataset({
            ...
            measures: [
                ...
                {
                    group: 1,
                    name: oBundle.getText("size"),
                    // 'name' is used as label in the Legend 
                    value: "{record_count}"    // 'value' defines the binding for the displayed value   
                },
                {
                    group: 2,
                    name: oBundle.getText("size2"),
                    // 'name' is used as label in the Legend 
        ...
        oVizFrame.setYAxis(new sap.viz.ui5.types.Axis({ label: { formatString: "u" } }));
        oVizFrame.setToolTip(new sap.viz.ui5.types.Tooltip({
            formatString: [[tooltipFormatString]]    // defined in global.js
        }));
        this.getTableSizes(this);
    },
    onHelpOpen: function () {
        var view = this.getView();
        view._bDialog = sap.ui.xmlfragment("sap.hana.democontent.epm.admin.ui.view.tileDialog", this    // associate controller with the fragment
);
            ...
            if (oModel.getProperty("/cb1")) {
                ...
                jQuery.ajax({
                    ...
                    success: function (myTxt) {
                        ...
                        var oUrl = "/sap/hana/xs/dt/base/file/sap/hana/democontent/epm/data/loads/MasterData.hdbti";
                        oController.reloadData(oUrl, oController);
                        oUrl = "/sap/hana/xs/dt/base/file/sap/hana/democontent/epm/data/loads/Util.hdbti";
                        oController.reloadData(oUrl, oController);
                    },
                ...
                jQuery.ajax({
                    ...
                    success: function (myTxt) {
                        ...
                        var oUrl = "/sap/hana/xs/dt/base/file/sap/hana/democontent/epm/data/loads/Sales.hdbti";
                        oController.reloadData(oUrl, oController);
                        oUrl = "/sap/hana/xs/dt/base/file/sap/hana/democontent/epm/data/loads/Purchase.hdbti";
                        oController.reloadData(oUrl, oController);
                    },
                    error: function (jqXHR, textStatus, errorThrown) {
                        onError(jqXHR.status, oBundle.getText("cb2"));
                    },
                    async: true
                });
                ...
                for (var i = 0; i < tableArray.length; i++) {
                    jQuery.ajax({
                        ...
                        dataType: 'text',
                        success: function (myTxt) {
                            oController.onResequenceComplete(myTxt, oController, tableArray[i], i >= tableArray.length - 1);
                        },
                        error: function (jqXHR, textStatus, errorThrown) {
                ...
                } else if (parseInt(oModel.getProperty('/SOVal')) !== 0) {
        ...
        $.ajax({
            ...
            success: function onComplete(data) {
                ...
                $.ajax({
                    ...
                    error: function (jqXHR, textStatus, errorThrown) {
                        onError(jqXHR.status, oBundle.getText("put_request"));
                    }
                });
            },
            error: function (jqXHR, textStatus, errorThrown) {
                onError(jqXHR.status, oBundle.getText("get_request"));
            }
        });
    },
    ...
    updateReplicateTimeBasedProgress: function () {
        ...
        oModel.setProperty('/displayValue', oBundle.getText("generatedTimeBased", [numericSimpleFormatter(totalOrdersCreated * 1000)]));
    },
    ...
    onReseedComplete: function (myTxt, oController) {
        ...
        oModel.setProperty('/displayValue', oBundle.getText("reloadedPG", [
            phase1.toString(),
            7
        ]));
    },
