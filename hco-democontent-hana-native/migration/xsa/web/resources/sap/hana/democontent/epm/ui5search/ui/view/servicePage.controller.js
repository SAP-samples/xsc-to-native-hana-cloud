...
sap.ui.controller("sap.shine.esh.view.servicePage", {
    ...
    getDataBindingCells: function (properties, resultTable) {
        ...
        for (var i = 0; i < properties.length; i++) {
            ...
            var cell = new sap.m.Text({ text: "{" + text + "}" });
    ...
    fillTable: function () {
        ...
        resultTable.bindItems({
            ...
            template: new sap.m.ColumnListItem({ cells: that.getDataBindingCells(properties, resultTable) }),
            parameters: { custom: { facets: 'all' } },
            events: {
    ...
    filterByFacet: function () {
        ...
        if (searchTerm !== "") {
            ...
            table.bindItems({
                path: "/" + oComponent.getProperty('entitySet'),
                template: new sap.m.ColumnListItem({ cells: that.getDataBindingCells(properties, table) }),
                parameters: {
                    custom: {
                        facets: 'all',
                        search: searchTerm
