sap.ui.controller("shine.democontent.epm.poworklist.view.Table", {
    ...
    onTBPress: function (oEvent, oController) {
        ...
        if (oEvent.getSource() === this.byId("btnConv")) {
            // xsjs will handle the content type and download will trigger automatically
            var aUrl = '/sap/hana/democontent/epm/services/LoadCurr.xsjs';
            ...
            jQuery.ajax({
                ...
                success: function (myTxt) {
                    ...
                    sap.ui.commons.MessageBox.show(oBundle.getText("loadCurrency"), "SUCCESS", oBundle.getText("titleCurrency"));
                },
                error: function (myTxt) {
                    sap.ui.core.BusyIndicator.hide();
                    sap.ui.commons.MessageBox.show(oBundle.getText("loadFailure"), "ERROR", oBundle.getText("titleCurrency"));
                }
            });
            //		window.open("/sap/hana/democontent/epm/services/LoadCurr.xsjs");
            return;
        }
        //Excel Download
        ...
        if (poId === undefined || poId === null) {
            sap.ui.commons.MessageBox.show(oBundle.getText("error_select"), "ERROR", oBundle.getText("error_action"));
        } else {
            //Supported Buttons - Delete and Edit
            switch (oEvent.getSource().getId()) {
            case this.byId("btnDelete").getId():
                sap.ui.commons.MessageBox.confirm(oBundle.getText("confirm_delete", [poId]), jQuery.proxy(function (bResult) {
                    this.deleteConfirm(bResult, this, poId);
                }, this), oBundle.getText("title_delete"));
                break;
    ...
    onMenuSelected: function (oEvent, oController) {
        ...
        if (poId === undefined || poId === null) {
            ...
            sap.ui.commons.MessageBox.confirm(oBundle.getText("confirm_po", [
                action,
                poId
            ]), jQuery.proxy(function (bResult) {
                this.approvalConfirm(bResult, this, poId, action);
            }, this), oBundle.getText("confirm_title", [action]));
        }
    },
    ...
    onDeleteSuccess: function (myTxt, oController) {
        ...
        sap.ui.commons.MessageBox.show(oBundle.getText("delete_success"), "SUCCESS", oBundle.getText("title_delete_success"));
    },
    //Approval Successful Event Handler
    onApprovalSuccess: function (myTxt, oController, action) {
        oController.refreshTable();
        sap.ui.commons.MessageBox.show(oBundle.getText("title_approve_success", [action]), "SUCCESS", oBundle.getText("title_approve_success"));
    },
    //Error Event Handler
    onErrorCall: function (jqXHR, textStatus, errorThrown) {
        if (jqXHR.status == '500') {
            sap.ui.commons.MessageBox.show(jqXHR.responseText, "ERROR", oBundle.getText("error_action"));
            return;
        } else {
            sap.ui.commons.MessageBox.show(jqXHR.statusText, "ERROR", oBundle.getText("error_action"));
            return;
        }
    },
    ...
    numericFormatter: function (val) {
        if (val == undefined) {
            ...
            var curRate = parseFloat(localStorage.getItem("CurRate"));
            ...
            if (curRate) {
                var prev = localStorage.getItem("PrevRate");
                if (prev) {
                    return oNumberFormat.format(val * curRate * parseFloat(prev));
                } else {
                    return oNumberFormat.format(val * curRate);
                }
            } else {
    ...
    currencyNameConverter: function (val) {
        var curcode = localStorage.getItem("CurCurrency");
        if (curcode) {
            return curcode;
        } else {
            return "EUR";
