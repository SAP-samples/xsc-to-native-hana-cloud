...
sap.ui.controller("spatial-demo.productsHeatMap", {
    ...
    onAfterRendering: function () {
        ...
        view.selector.bindItems({
            path: '/ProductDetails',
            parameters: { select: 'PRODUCTID,PRODUCT_NAME' },
    ...
    onProductSelected: function (oEvent, oController) {
        oController.loadDataForProduct(oController.getView().selector.getSelectedKey(), oController.getView(), oController);
    },
    loadDataForProduct: function (productID, view, oController) {
        view.oSalesChart.bindRows({
            path: '/ProductSales',
            filters: [new sap.ui.model.Filter("PRODUCT_PRODUCTID", "EQ", productID)]
        });
        OData.read("/sap/hana/democontent/epm/spatial/services/productSales.xsodata/ProductRegionQuantity?$filter=PRODUCT_PRODUCTID eq '" + productID + "'", function (data, request) {
            ...
            if (data.results && data.results.length > 0) {
                ...
                oController.heatmapLayer = new H.map.layer.TileLayer(oController.heatmapProvider, { opacity: 0.8 });
