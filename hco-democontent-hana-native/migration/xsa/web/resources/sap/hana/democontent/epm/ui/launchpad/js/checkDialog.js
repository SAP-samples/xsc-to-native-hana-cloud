...
sap.account.CheckDialog.prototype.open = function () {
    ...
    // vspace
    oContentMatrix.addRow(new sap.ui.commons.layout.MatrixLayoutRow({ height: '20px' }));
    ...
    oCellStatus = new sap.ui.commons.layout.MatrixLayoutCell({ hAlign: sap.ui.commons.layout.HAlign.Right });
    oCheckDialog.tdStatusLayout = new sap.ui.layout.HorizontalLayout({ content: [new sap.m.BusyIndicator({ size: "1.4em" })] });
    ...
    oCellStatus = new sap.ui.commons.layout.MatrixLayoutCell({ hAlign: sap.ui.commons.layout.HAlign.Right });
    oCheckDialog.synonymLayout = new sap.ui.layout.HorizontalLayout({ content: [new sap.m.BusyIndicator({ size: "1.4em" })] });
        ...
        jQuery.ajax({
            ...
            success: function (data) {
                ...
                if (response.hasTimeData) {
                    oCheckDialog.tdStatusLayout.addContent(new sap.ui.commons.Image({ src: './launchpad/images/green_tick.png' }));
                } else {
                    oCheckDialog.timeBtn.setEnabled(true);
                    oCheckDialog.tdStatusLayout.addContent(new sap.ui.commons.Image({ src: './launchpad/images/red_cross.png' }));
                }
                // synonyms check
                oCheckDialog.synonymLayout.removeAllContent();
                if (response.synonymsPresent) {
                    oCheckDialog.synonymLayout.addContent(new sap.ui.commons.Image({ src: './launchpad/images/green_tick.png' }));
                } else {
                    oCheckDialog.synonymBtn.setEnabled(true);
                    oCheckDialog.synonymLayout.addContent(new sap.ui.commons.Image({ src: './launchpad/images/red_cross.png' }));
                }
            },
            error: function () {
    ...
    function createHeaderRow() {
        ...
        var oHorizontalLayout = new sap.ui.commons.layout.HorizontalLayout({ content: [textView] });
