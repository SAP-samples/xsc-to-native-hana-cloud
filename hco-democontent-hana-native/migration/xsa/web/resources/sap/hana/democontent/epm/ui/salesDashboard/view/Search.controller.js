sap.ui.controller("sap.hana.democontent.epm.salesdashboard.view.Search", {
    ...
    setFilter: function (attribute) {
        ...
        var sort1 = new sap.ui.model.Sorter("COMPANYNAME", true);
        oTable = sap.ui.getCore().byId("details--soHeader");
        oTable.bindRows({
        ...
        var oTableItem = sap.ui.getCore().byId("details--soItemTable");
    ...
    loadFilter: function (oEvent) {
        ...
        if (gSearchParam === "*" || gSearchParam === "") {
            sap.ui.controller("sap.hana.democontent.epm.salesdashboard.view.Search").emptyFilter();
        } else {
            var aUrl = '/sap/hana/democontent/epm/services/soWorklistQuery.xsjs?cmd=filter' + '&query=' + encodeURI(oEvent.getParameter("value")) + '&page=1&start=0&limit=25';
            jQuery.ajax({
                url: aUrl,
                method: 'GET',
                dataType: 'json',
...
function onLoadFilter(myJSON) {
    ...
    sap.ui.controller("sap.hana.democontent.epm.salesdashboard.view.Search").setFilter(aSuggestions);
}
function onErrorCall(jqXHR, textStatus, errorThrown) {
    sap.ui.commons.MessageBox.show(jqXHR.responseText, "ERROR", sap.app.i18n.getText("ERROR_ACTION"));
    return;
}