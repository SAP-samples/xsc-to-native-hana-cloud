sap.ui.controller("spatial-demo.sales-analysis", {
    ...
    onAfterRendering: function () {
        ...
        var svgMarkup = '<svg  width="30" height="30" xmlns="http://www.w3.org/2000/svg">' + '<circle cx="15" cy="15" r="10" stroke="black" stroke-width="5" fill="white" />' + '</svg>';
        var polyLineIcon = new H.map.Icon(svgMarkup);
        // Obtain the default map types from the platform object:
        var defaultLayers = sap.app.platform.createDefaultLayers();
        // Instantiate (and display) a map object:
        oController.map = new H.Map(document.getElementById("__splitter1_firstPane"), defaultLayers.normal.map, {
            zoom: 5,
            center: {
                lat: 50.5,
                lng: 3.4
        ...
        oController.map.addEventListener('tap', function (evt) {
            ...
            var coord = oController.map.screenToGeo(evt.currentPointer.viewportX, evt.currentPointer.viewportY);
            ...
            if (!polyLineStrip && length == 1) {
                ...
                polyLine = new H.map.Polyline(polyLineStrip, { style: { lineWidth: 4 } });
            ...
            var polyLineMarker = new H.map.Marker(coord, { icon: polyLineIcon });
        ...
        var oText = new sap.ui.core.HTML({ content: sap.app.i18n.getText("INIT_HELP") });
        ...
        for (var j = 0; j < data.topBuyers.length; j++) {
            ...
            custMarker.addEventListener("tap", function (evt) {
                ...
                var bubble = new H.ui.InfoBubble(marker.getPosition(), {
                    // read custom data
                    content: '<div>' + '<h3>' + marker.getData().companyName + ' ' + marker.getData().legalForm + '</h3>' + '<div id=\'chartHolder\'></div></div>'
                });
                // show info bubble
