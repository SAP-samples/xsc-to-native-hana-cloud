...
sap.ui.core.UIComponent.extend("sap.hana.democontent.epm.admin.Component", {
    metadata: {
        ...
        rootView: "sap.hana.democontent.epm.admin.ui.view.App",
        config: {
            resourceBundle: "ui/i18n/messagebundle.properties",
            serviceConfig: {
                name: "Get Size",
                serviceUrl: "/sap/hana/democontent/epm/admin/DataGen.xsjs?cmd=getSize"
            }
        }
    },
    init: function () {
    ...
    getSessionInfo: function () {
        var aUrl = '/sap/hana/democontent/epm/admin/DataGen.xsjs?cmd=getSessionInfo';
        this.onLoadSession(JSON.parse(jQuery.ajax({
            url: aUrl,
            method: 'GET',
            dataType: 'json',
            async: false
    ...
    onLoadSession: function (myJSON) {
        for (var i = 0; i < myJSON.session.length; i++) {
            ...
            mConfig.setProperty("/UserName", myJSON.session[i].UserName);
        }
    },
