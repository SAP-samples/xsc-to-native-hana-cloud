...
shine.usercrud.User.prototype.persistIntoTable = function(){
    try{
        ...
        var query = 'select "SAP_HANA_DEMO"."sap.hana.democontent.epm.data::userSeqId".NEXTVAL as "PERSNO" from dummy';
        var rs = connection.executeQuery(query);
        var persNo = '';
        if(rs.length > 0) {
            persNo = rs[0].PERSNO;
        }
        query = 'insert into SAP_HANA_DEMO."sap.hana.democontent.epm.data::User.Details" values(?,?,?,?)';
