...
function checkSQLInjectionSafe(checkVariable) 
{
    var conn = $.db.getConnection();
    var pstmt;
    var rs;
    var SQLCheckbit;
    var query = 'select IS_SQL_INJECTION_SAFE('+ checkVariable +') FROM DUMMY';
    pstmt = conn.prepareStatement(query);
    rs = pstmt.executeQuery();
        while (rs.next()) {
            SQLCheckbit = rs.getInteger(1).toString();
        }
    return SQLCheckbit;
}
...
function recordSetToText(rs, bHeaders, delimiter) {
    bHeaders = bHeaders !== undefined ? bHeaders : true;
    delimiter = delimiter !== undefined ? delimiter : '\t'; //Default to Tab Delimited
...
function get_session_variable(name, application) {
    ...
    var results = proc.get_session_variable(sessionId, name, application, conn);
    if (results.EX_SVARIABLE[0] !== undefined) {
        output = results.EX_SVARIABLE[0].DATA;
        conn.commit();
        conn.close();
        return output;
    } 
...
function get_session_variables(application) {
    var sessionId = getSessionId();
    var conn = $.db.getConnection();
    var results = proc.get_session_variables(sessionId, application, conn);
    var jsonOut = results.EX_SVARIABLES;
    conn.commit();
    conn.close();
    return jsonOut;
}
...
function set_session_variable(name, application, value, expiry) {
    ...
    var conn = $.db.getConnection();
    proc.set_session_variable(sessionId, name, application, expiry, value.toString(), conn);
...
function get_application_variable(name, application) {
    ...
    var results = proc.get_application_variable(name, application, conn);
    if (results.EX_SVARIABLE[0] !== undefined) {
        output = results.EX_SVARIABLE[0].DATA;
        conn.commit();
        conn.close();
...
function get_application_variables(application) {
    var conn = $.db.getConnection();
    var results = proc.get_application_variables(application, conn);
    var jsonOut = results.EX_SVARIABLE;
    conn.commit();
    conn.close();
    return jsonOut;
}
...
function set_application_variable(name, application, value, expiry) {
    ...
    var conn = $.db.getConnection();
    proc.set_application_variable(name, application, expiry, value.toString(), conn);
