...
function addScheduleXSJS() {
    ...
    rs = conn.executeQuery('SELECT ID from "SAP_HANA_DEMO"."sap.hana.democontent.epm.data::JobsDemo.RuntimeSchedules" where NAME = ?', jobName);
...
function clearJobLogs(){
    ...
    var conn = $.hdb.getConnection();
    try{
        conn.executeUpdate('DELETE FROM "SAP_HANA_DEMO"."sap.hana.democontent.epm.data::JobsDemo.Details"');
        conn.commit();
        conn.close();
    }
    catch(e){
