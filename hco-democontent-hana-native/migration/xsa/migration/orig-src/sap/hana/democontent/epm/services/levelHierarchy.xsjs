...
try
{
  ...
  var query = 'SELECT  "COUNTRY",SUM("TOTAL_SALES") AS "TOTAL_SALES" FROM "_SYS_BIC"."sap.hana.democontent.epm.models/SALESORDER_DYNAMIC_JOIN"'+' WHERE "COUNTRY" IN (SELECT "QUERY_NODE_NAME"'+
           ' FROM "_SYS_BIC"."sap.hana.democontent.epm.models/SALESORDER_DYNAMIC_JOIN/REGION_COUNTRY_LEVEL/hier/REGION_COUNTRY_LEVEL"' +' WHERE PRED_NODE = ?)'+
            'GROUP BY "COUNTRY"';                
