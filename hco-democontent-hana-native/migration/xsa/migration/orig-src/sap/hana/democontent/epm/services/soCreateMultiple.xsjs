...
query = 'SELECT \"SAP_HANA_DEMO\".\"sap.hana.democontent.epm.data::salesOrderId\".NEXTVAL as OverallId from Dummy';
...
query = 'SELECT COMPANYNAME from "SAP_HANA_DEMO"."sap.hana.democontent.epm.data::MD.BusinessPartner" where PARTNERID = ?';
...
if (so_items) {
    ...
    for (i; i < so_items.length; i++) {
        ...
        query = 'SELECT PRICE from "SAP_HANA_DEMO"."sap.hana.democontent.epm.data::MD.Products" where PRODUCTID = ?';
        ...
        query = "create local temporary table \"SAP_HANA_DEMO\".#TT_IN(PARTNERID nvarchar(10),COMPANYNAME nvarchar(80),PRODUCTID nvarchar(10))";
        conn.executeUpdate(query);

        //Insert values into the local temporary table for bp id,company name and product id for the tax calculation
        try {
            query = "insert into \"SAP_HANA_DEMO\".#TT_IN values (?,?,?)";
        ...
        taxCalculator = conn.loadProcedure('_SYS_BIC', 'sap.hana.democontent.epm.models/DT_TAX_CALCULATION');
        ...
        query = 'insert into "SAP_HANA_DEMO"."sap.hana.democontent.epm.data::SO.Item"' + " values(?,?,?,'','EUR',?,?,?,'I','',?,'EA','')";
        finalItemId = '';
        if (item_id >= 100) {
        ...
        query = "drop table \"SAP_HANA_DEMO\".#TT_IN";
...
query = 'insert into \"SAP_HANA_DEMO\".\"sap.hana.democontent.epm.data::SO.Header\"' 
