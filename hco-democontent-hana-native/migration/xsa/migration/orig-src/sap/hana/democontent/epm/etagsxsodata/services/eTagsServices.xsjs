function readUserIdSequence(){
	...
	rs = conn.executeQuery('SELECT \"SAP_HANA_DEMO\".\"sap.hana.democontent.epm.data::userSeqId\".NEXTVAL as OverallId from Dummy');
	overAllId = '';
	if (rs.length !== 0) {
...
function clearJobLogs(){
    ...
    var conn = $.hdb.getConnection();
    try{
        conn.executeUpdate('DELETE FROM "SAP_HANA_DEMO"."sap.hana.democontent.epm.data::User.Details"');
        conn.commit();
        conn.close();
    }
    catch(e){
